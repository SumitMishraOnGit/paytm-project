  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const balanceResponse = await axios.get("api/v1//account/balance", config);
        setBalance(balanceResponse.data.balance);

        const userResponse = await axios.get("api/v1//user/getUser", config);
        setFirstName(userResponse.data.firstName);

        const usersResponse = await axios.get("api/v1//user/bulk?filter=", config);
